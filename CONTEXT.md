...
2026-09-24 最新优先级：用户要求先完成 Windows，macOS 和 Linux 搁置。当前变更 windows-rdp-launch 为 0.2.1 增加从设备列表打开 Windows 系统远程桌面，直接复用现有 RDP 设置；任务执行端配对仍用于命令/脚本。此轮 CI 与新安装包只面向 Windows。

桌面设备目录、配对远程任务和本机应用维护。Tailscale 提供设备网络与身份观测；本应用不嵌入 VPN 实现，也不把设备在线视为远程授权。

用户于 2026-09-24 撤回了“Agent 仅能修复、不能操控”的限制。0.2.0 已实现原 PRD 中用户主动发起的远程命令、脚本、日志、取消和结果功能，并公开发布三平台未签名测试安装包；四平台 CI 全部通过，Windows 发布产物已实际安装并完成本机配对执行验证。原生窗口完整流程、登录自启和跨机器验收仍待完成，详见 docs/validation.md。0.1.0 是本机修复基础。窗口、安装器和界面使用 xiangwriter远程器 名称，仓库名称固定 tailscale-ui。
