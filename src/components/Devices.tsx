...
export function Devices({state,onAdd,onRemote}:{state:TailTaskState;onAdd:()=>void;onRemote?:()=>void}) {
  ...
  return <>
    ...
    <div className={'device-grid '+(state.selected?'has-selection':'')}>
      ...
      {state.selected?<DeviceDetail key={state.selected.node_id} state={state} onRemote={onRemote}/>:<aside className="panel selection-placeholder"><NetworkArt/><div><span className="eyebrow">你的设备，一目了然</span><h2>选择设备，查看详情</h2><p>查看地址、连接状态与本地别名。<br/>每台设备都有清晰、独立的记录。</p></div></aside>}
    </div>
  </>;
}
